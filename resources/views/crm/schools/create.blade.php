            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('schools.store') }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <x-text-input id="short_name" name="short_name" class="block mt-1 w-full" :value="old('short_name')" />
                    </div>
                    <div>
                        <x-input-label for="address" :value="__('Address')" />
                        <textarea id="address" name="address" rows="2" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('address') }}</textarea>
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <x-input-label for="city" :value="__('City')" />
                            <x-text-input id="city" name="city" class="block mt-1 w-full" :value="old('city')" />
                        </div>
                        <div>
                            <x-input-label for="state" :value="__('State')" />
                            <x-text-input id="state" name="state" class="block mt-1 w-full" :value="old('state')" />
                        </div>
                    </div>
                    <div>
                        <x-input-label for="contact_email" :value="__('Contact email (optional)')" />
                        <x-text-input id="contact_email" name="contact_email" type="email" class="block mt-1 w-full" :value="old('contact_email')" />
                    </div>
