<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-2">
            ...
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Edit Session') }}</h2>
        </div>
    </x-slot>
            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('sessions.update', $session) }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <x-text-input id="name" name="name" class="block mt-1 w-full" :value="old('name', $session->name)" required />
                        <x-input-error :messages="$errors->get('name')" class="mt-1" />
                    </div>
                    <div>
                        <label class="inline-flex items-center">
                            <input type="checkbox" name="is_current" value="1" class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" {{ old('is_current', $session->is_current) ? 'checked' : '' }}>
