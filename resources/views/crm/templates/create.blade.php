<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-2">
            ...
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Add Aisensy Template') }}</h2>
        </div>
    </x-slot>
    ...
    <div class="py-6">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                <p class="text-sm text-gray-600 mb-4">{{ __('Template name must match exactly the campaign name in your Aisensy dashboard.') }}</p>
                <form method="POST" action="{{ route('templates.store') }}" class="space-y-4">
                    ...
                    @endif
                    <div>
                        <x-input-label for="name" :value="__('Template / campaign name (Aisensy)')" />
                        <x-text-input id="name" name="name" class="block mt-1 w-full" :value="old('name')" required placeholder="e.g. FEE_REMINDER" />
                    ...
                    <div>
                        ...
                        <textarea id="body" name="body" rows="4" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm" placeholder="Hello @{{1}}, thank you for registering at @{{2}}. Class: @{{3}}.">{{ old('body') }}</textarea>
                        <p class="text-xs text-slate-500 mt-1">{{ __('Paste your WhatsApp template text. Use') }} @{{1}}, @{{2}}, @{{3}}… {{ __('for parameters. Campaign reports will show the exact message sent.') }}</p>
                    </div>
                    <div>
                        <x-input-label for="param_count" :value="__('Number of template parameters (0–4)')" />
                        <select id="param_count" name="param_count" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            @for ($i = 0; $i <= 4; $i++)
                                <option value="{{ $i }}" {{ (int) old('param_count', 0) === $i ? 'selected' : '' }}>{{ $i }}</option>
                            @endfor
                        </select>
                    </div>
                    <div id="param-mappings" class="space-y-2">
                        @for ($i = 0; $i <= 4; $i++)
                            <div class="param-row" data-index="{{ $i }}" style="{{ $i >= (int) old('param_count', 0) ? 'display:none' : '' }}">
                                ...
                                <select name="param_{{ $i }}" id="param_{{ $i }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    @foreach ($paramOptions as $value => $label)
                                        <option value="{{ $value }}" {{ old('param_'.$i) === $value ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
                            </div>
                        @endfor
